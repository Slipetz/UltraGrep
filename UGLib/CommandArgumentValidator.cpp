...
CommandArgumentValidator::CommandArgumentValidator(int argc, char * argv[]) : anyInvalidArgument(false)
{
	...
	if (argc == 3) {
		...
		GenerateRegex(argv[2]);
		valPackage.fileVariables = FileVariables(validRgx);
	}
	else if (argc == 4) {
		...
		if (vbCheck.compare("-v") == 0 || vbCheck.compare("-V") == 0) {
			...
			GenerateRegex(argv[3]);
			valPackage.fileVariables = FileVariables(validRgx, { ".txt" }, true);
		}
		else {
			...
			GenerateRegex(argv[2]); //Second argument is the regex
			vector<string> parsedExtensions = ParseExtensions(argv[3]);
			valPackage.fileVariables = FileVariables(validRgx, parsedExtensions);
		}
	}
	else 
	{
		...
		valPackage.fileVariables = FileVariables(validRgx, parsedExtensions, verbose);
	}
}
...
void CommandArgumentValidator::GenerateRegex(string const & rgx)
{
	try {
		regex rgxTest(rgx);
		validRgx = rgxTest;
		...
		cerr << "Regex exception - Invalid Regex Given: " << rgx << endl;
		anyInvalidArgument = true;
	}
}
